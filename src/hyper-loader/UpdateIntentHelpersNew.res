...
let performUpdateIntent = async (
  ...
  ~selectorString,
  ~shouldWaitForReady,
  ~merchantEvent: HyperLoaderLogger.merchantEvent=HyperLoaderLogger.PaymentSession(UpdateIntent),
) => {
  if isUpdateIntentInProgress.contents {
    ...
    let runUpdateIntent = async () =>
      try {
        ...
        | None =>
          ...
          HyperLoaderLogger.logLifecycle(~event=UpdateSdk)
    ...
    let response = await HyperLoaderLogger.observeMerchant(
      ~event=merchantEvent,
