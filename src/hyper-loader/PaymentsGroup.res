...
let makeCardForm = (~config: groupConfig): Types.cardForm => {
  ...
  HyperLoaderLogger.logState(~event=CardFormMounted({scope: PaymentForm}))
  ...
  let deinit = (): unit =>
    HyperLoaderLogger.observeMerchantSync(~event=HyperLoaderLogger.CardForm(Deinit), ~call=() => {
      HyperLoaderLogger.logState(~event=CardFormUnmounted({scope: PaymentForm}))
      fieldsRef.contents
      ->Dict.valuesToArray
      ->Array.forEach(entry => {
