...
let makeCardForm = (~config: groupConfig): Types.cardForm => {
  ...
  let sdkSessionId = config.sdkSessionId->Option.getOr("")
  SdkLogger.logState(~event=CardFormMounted({scope: PaymentForm}))
  ...
  let create = (fieldType: string, options: JSON.t): fieldHandle =>
    HyperLoaderLogger.logMerchantCall(
      ...
      ~call=() =>
        ...
        | _ =>
          SdkLogger.logState(~event=CardFieldMounted({scope: PaymentForm, field: fieldType}))
    ...
    if attemptsClientSecretMutation || attemptsConfirmParamsMutation {
      HyperLoaderLogger.logMerchantIssue(
        ...
        ~details=[
          (
            "field",
            (attemptsClientSecretMutation ? "clientSecret" : "confirmParams")->JSON.Encode.string,
          ),
          ("reason", "immutable_after_mount"->JSON.Encode.string),
        ],
      )
      Console.warn(
