...
let make = (~sessionObj: SessionsType.token) => {
  ...
  let mountPaypalSDK = () => {
    ...
    let intentParam = if paypalIntent !== "" {
      ...
      SdkLogger.logLifecycle(~event=WalletFlowFailed({reason: MissingIntent}))
      ""
    }
    ...
    let currencyParam = if currency !== "" {
      ...
      SdkLogger.logLifecycle(~event=WalletFlowFailed({reason: MissingCurrency}))
      ""
    }
  ...
  React.useEffect(() => {
    ...
    | exn =>
      SdkLogger.logLifecycle(
        ~event=WalletFlowFailed({reason: SdkMountFailed}),
