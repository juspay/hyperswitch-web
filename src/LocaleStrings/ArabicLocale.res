let localeStrings: LocaleStringTypes.localeStrings = {
  ...
  qrCodeValidityText: `رمز QR صالح لمدة 15 دقيقة القادمة، يرجى عدم إغلاق الصفحة حتى تتم عملية الدفع بنجاح، وبعدها ستتم إعادة توجيهك تلقائيًا.`,
  doneText: `تم`,
  moreText: `المزيد`,
  addBankDetailsConfirmText: `يرجى إضافة تفاصيل البنك ثم تأكيد الدفع باستخدام طرق الدفع المضافة.`,
  addBankDetailsButtonText: `إضافة تفاصيل البنك`,
  bankDebitStepsText: pmt =>
    `الخصم المباشر ${pmt} يتطلب خطوتين للدفع. يرجى اتباع التعليمات:`,
  bankDebitStep1Text: `يرجى النقر على إضافة تفاصيل البنك والمتابعة لإضافة تفاصيل حسابك المصرفي.`,
  bankDebitStep2Text: `بعد التحقق، ستظهر الحسابات المضافة. يمكنك اختيار الحساب الذي تريد الدفع منه والنقر على زر الدفع.`,
  removeAccountText: `إزالة الحساب`,
  bankAccountDisplayText: last4 => `بنك  **** ${last4}`,
  microDepositsInitiatedText: `تم بدء الإيداعات الصغيرة`,
  microDepositsExpectText: last4 =>
    `توقع إيداعًا بقيمة $0.01 في الحساب المنتهي بـ **** ${last4} خلال 1-2 أيام عمل ورسالة بريد إلكتروني تتضمن تعليمات إضافية للتحقق من حسابك.`,
  bankStatementDisplayText: last4 => `**** ${last4}  كشف حساب بنكي`,
  transactionText: `المعاملة`,
  amountText: `المبلغ`,
  typeText: `النوع`,
  bankDetailsHeadingText: `تفاصيل البنك`,
  accountHolderNameLabel: `اسم صاحب الحساب`,
  accountHolderNamePlaceholder: `مثال: محمد أحمد`,
  accountTypeLabel: `نوع الحساب`,
  errorOccurredText: `حدث خطأ`,
  bankTransferTitleText: title => `تحويل بنكي ${title}`,
  useDetailsToTransferText: `استخدم هذه التفاصيل لتحويل المبلغ`,
  bankAccountDetailsText: `تفاصيل الحساب المصرفي`,
  bankTransferDisclaimerText: `يرجى تدوين هذه التفاصيل قبل إغلاق هذه النافذة. لن تتمكن من إنشاء هذه التفاصيل مرة أخرى.`,
  voucherGeneratedText: pm =>
    `تم إنشاء قسيمة ${pm} بنجاح! إذا لم يبدأ تنزيل المستند تلقائيًا، انقر `,
  hereText: `هنا`,
  toDownloadItText: ` لتنزيله.`,
  barCodeReferenceText: `مرجع الرمز الشريطي: `,
  voucherDisclaimerText: `يرجى عدم إغلاق الصفحة حتى يتم تنزيل القسيمة بنجاح، وبعدها ستتم إعادة توجيهك تلقائيًا.`,
  payWithText: `الدفع باستخدام`,
  notYouText: `لست أنت؟`,
  ctpSwitchIdentifierText: `أدخل بريدًا إلكترونيًا أو رقم هاتف جديدًا للوصول إلى مجموعة مختلفة من البطاقات المرتبطة.`,
  phoneLabel: `الهاتف`,
  enterEmailPlaceholder: `أدخل البريد الإلكتروني`,
  mobileNumberPlaceholder: `رقم الهاتف المحمول`,
  switchIdButtonText: `تبديل المعرّف`,
  ctpConsentSharingText: (dpaName, cardBrand) =>
    `بالمتابعة، سيشارك ${dpaName} تفاصيل بطاقتك وعنوان الفوترة والبريد الإلكتروني مع ${cardBrand} للسماح لك بالتسجيل بأمان في Click to Pay لعمليات دفع أسرع.`,
  learnMoreText: `اعرف المزيد`,
  ctpSaveInfoText: cardBrand => `حفظ معلوماتي لدى ${cardBrand} `,
  ctpFasterCheckoutText: `لعملية دفع أسرع وأكثر أمانًا`,
  ctpVerifyIdentityText: `سيتم استخدام بريدك الإلكتروني أو رقم هاتفك للتحقق من هويتك.`,
  ctpDataRatesText: `قد تُطبّق رسوم الرسائل/البيانات.`,
  ...
  processingPaymentText: `جارٍ معالجة دفعتك...`,
  redirectedToNewTabText: `تم توجيهك إلى علامة تبويب جديدة لإكمال عملية الدفع. سيتم تحديث الحالة تلقائيًا`,
  ...
  errorBackShortlyText: `سنعود إليك قريبًا :)`,
  tryAnotherPaymentMethodText: `جرّب طريقة دفع أخرى :)`,
}
