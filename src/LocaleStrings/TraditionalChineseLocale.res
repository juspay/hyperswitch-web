let localeStrings: LocaleStringTypes.localeStrings = {
  ...
  qrCodeValidityText: `此 QR 碼在接下來的 15 分鐘內有效，請勿在付款成功完成前關閉頁面，完成後您將被自動重新導向。`,
  doneText: `完成`,
  moreText: `更多`,
  addBankDetailsConfirmText: `請新增銀行資料，然後使用已新增的付款方式確認付款。`,
  addBankDetailsButtonText: `新增銀行資料`,
  bankDebitStepsText: pmt =>
    `${pmt} 銀行扣帳需要 2 個步驟完成付款。請按照以下指示操作：`,
  bankDebitStep1Text: `請點擊「新增銀行資料」並繼續新增您的銀行帳戶資料。`,
  bankDebitStep2Text: `驗證完成後，您將看到已新增的帳戶。選擇您要用來付款的帳戶，然後點擊付款按鈕。`,
  removeAccountText: `移除帳戶`,
  bankAccountDisplayText: last4 => `銀行  **** ${last4}`,
  microDepositsInitiatedText: `微額存款已啟動`,
  microDepositsExpectText: last4 =>
    `預計在 1-2 個工作日內，尾號為 **** ${last4} 的帳戶將收到一筆 $0.01 的存款，以及一封包含驗證帳戶額外說明的電子郵件。`,
  bankStatementDisplayText: last4 => `**** ${last4}  銀行對帳單`,
  transactionText: `交易`,
  amountText: `金額`,
  typeText: `類型`,
  bankDetailsHeadingText: `銀行資料`,
  accountHolderNameLabel: `帳戶持有人姓名`,
  accountHolderNamePlaceholder: `例如：王大明`,
  accountTypeLabel: `帳戶類型`,
  errorOccurredText: `發生錯誤`,
  bankTransferTitleText: title => `${title} 銀行轉帳`,
  useDetailsToTransferText: `使用以下資料進行轉帳`,
  bankAccountDetailsText: `銀行帳戶資料`,
  bankTransferDisclaimerText: `請在關閉此視窗前記下這些資料。您將無法再次產生這些資料。`,
  voucherGeneratedText: pm =>
    `${pm} 憑證已成功產生！如果文件未自動開始下載，請點擊`,
  hereText: `這裡`,
  toDownloadItText: `以下載。`,
  barCodeReferenceText: `條碼參考編號：`,
  voucherDisclaimerText: `請勿在成功下載憑證前關閉頁面，下載完成後您將被自動重新導向。`,
  payWithText: `使用以下方式付款`,
  notYouText: `不是您本人？`,
  ctpSwitchIdentifierText: `輸入新的電子郵件或手機號碼以存取另一組已連結的卡片。`,
  phoneLabel: `電話`,
  enterEmailPlaceholder: `輸入電子郵件`,
  mobileNumberPlaceholder: `手機號碼`,
  switchIdButtonText: `切換身分`,
  ctpConsentSharingText: (dpaName, cardBrand) =>
    `繼續操作即表示 ${dpaName} 將與 ${cardBrand} 分享您的卡片資料、帳單地址和電子郵件，以便您安全註冊 Click to Pay，享受更快速的結帳體驗。`,
  learnMoreText: `瞭解更多`,
  ctpSaveInfoText: cardBrand => `將我的資訊儲存至 ${cardBrand} `,
  ctpFasterCheckoutText: `以獲得更快速且安全的結帳體驗`,
  ctpVerifyIdentityText: `您的電子郵件或手機號碼將用於驗證您的身分。`,
  ctpDataRatesText: `可能適用訊息/數據費率。`,
  ctpRememberMeText: `在此瀏覽器中記住我`,
  ctpRememberMeTooltipLine1: `被記住後，使用 Click to Pay 付款時將跳過驗證並安全存取您已儲存的卡片。`,
  ctpRememberMeTooltipLine2: `不建議在公用或共用裝置上使用，因為這會使用 Cookie。`,
  ctpTermsConsentText: cardBrand => `繼續操作即表示您同意 ${cardBrand} 的`,
  termsText: `條款`,
  ctpPrivacyConsentText: `並瞭解您的資料將依據`,
  privacyNoticeText: `隱私權聲明`,
  loadingText: `載入中...`,
  processingPaymentText: `我們正在處理您的付款...`,
  redirectedToNewTabText: `您已被重新導向至新分頁以完成付款。狀態將自動更新`,
  addNewCardText: `新增卡片`,
