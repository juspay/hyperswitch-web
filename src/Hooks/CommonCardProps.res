...
let useCardForm = (
  ...
  let changeCardNumber = ev => {
    ...
    SdkLogger.logUser(~event=InputFieldChanged({field: "card_number"}))
  ...
  let changeCardExpiry = ev => {
    let val = ReactEvent.Form.target(ev)["value"]
    SdkLogger.logUser(~event=InputFieldChanged({field: "card_expiry"}))
  ...
  let changeCVCNumber = ev => {
    let val = ReactEvent.Form.target(ev)["value"]
    SdkLogger.logUser(~event=InputFieldChanged({field: "card_cvc"}))
  ...
  let changeZipCode = ev => {
    let val = ReactEvent.Form.target(ev)["value"]
    SdkLogger.logUser(~event=InputFieldChanged({field: "zip_code"}))
    setZipCode(_ => val)
  }
