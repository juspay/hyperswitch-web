...
let make = () => {
  ...
  let handleIframeContentLoaded = () => {
    ...
    if iframeId->String.length > 0 && !isThreeDSMethodCompletionFired.current {
      SdkRuntimeLogger.logLifecycle(~event=ThreeDsMethodResult, ~paymentMethod=Card(Unspecified), ~message="Y")
  ...
  let handleOnError = value => {
    SdkRuntimeLogger.logLifecycle(~event=ThreeDsMethodFailed, ~paymentMethod=Card(Unspecified), ~message=value)
    stateMetadataRef.current
    ->Utils.getDictFromJson
