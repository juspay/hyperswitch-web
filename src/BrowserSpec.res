...
// Browser info function specifically for Interac bank redirect
let getBrowserInfoForInterac = () => {
  let colorDepth =
    [1, 4, 8, 15, 16, 24, 32, 48]->Array.includes(screen.colorDepth) ? screen.colorDepth : 24
  ...
  [
    ...
    ("java_enabled", true->JSON.Encode.bool),
    ("java_script_enabled", true->JSON.Encode.bool),
    ("ip_address", "127.0.0.1"->JSON.Encode.string), // Placeholder since IP can't be detected client-side
  ]->Utils.getJsonFromArrayOfJson
}
