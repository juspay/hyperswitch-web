...
// DOM manipulation bindings (avoids document.write which changes URL)
@get external getHead: document => Nullable.t<element> = "head"
@get external getBody: document => Nullable.t<element> = "body"
@set external setInnerHTML: (element, string) => unit = "innerHTML"
@set external setTitle: (document, string) => unit = "title"

// The CSS styles for the loader
let loaderStyles = `
    * {
        margin: 0;
        padding: 0;
...
// Uses direct DOM manipulation instead of document.write() to avoid URL changes
let injectLoader = (windowRef: Nullable.t<Types.window>) => {
  switch windowRef->Nullable.toOption {
  | Some(win) => {
      let doc = win->getDocument
      doc->setTitle("Payment Processing")

      // Inject styles into head
      switch doc->getHead->Nullable.toOption {
      | Some(head) =>
        head->setInnerHTML(
          `<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>${loaderStyles}</style>`,
        )
      | None => ()
      }

      // Inject content into body
      switch doc->getBody->Nullable.toOption {
      | Some(body) => body->setInnerHTML(loaderBodyContent)
      | None => ()
      }
    }
  | None => ()
