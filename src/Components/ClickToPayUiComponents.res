...
module OtpInput = {
  ...
  let make = (~getCards: string => promise<unit>, ~setIsClickToPayRememberMe) => {
    ...
    let callBacks = {
      otpChanged: ev => {
        SdkRuntimeLogger.logUser(~event=InputFieldChanged, ~message="Click to Pay OTP changed")
      ...
      continueClicked: _ => {
        SdkRuntimeLogger.logUser(
          ...
          ~message="Click to Pay OTP continue clicked",
        )
      ...
      resendClicked: _ => {
        SdkRuntimeLogger.logUser(~event=ViewToggled, ~message="Click to Pay OTP resend clicked")
        setClickToPayConfig(prev => {
          ...prev,
          otpError: "",
        })
