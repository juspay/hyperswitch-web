module RffPhoneNumberPaymentInput = {
  @react.component
  let make = (~numberName: string, ~codeName: string) => {
    ...
    let numberField: ReactFinalForm.fieldProps<ReactEvent.Focus.t> = ReactFinalForm.useField(
      numberName,
    )
    let codeField: ReactFinalForm.fieldProps<ReactEvent.Focus.t> = ReactFinalForm.useField(codeName)
