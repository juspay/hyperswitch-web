...
let usePaymentIntent = (optLogger, paymentType) => {
  ...
  let setIsManualRetryEnabled = Recoil.useSetRecoilState(isManualRetryEnabled)
  let isSubscriptionFlow = keys.initSubscriptionsFlow->Option.getOr(false)
  ...
  (
    ...
    let bodyArr = isSubscriptionFlow
      ? [("payment_details", bodyArr->Utils.getJsonFromArrayOfJson)]
      : bodyArr
    switch keys.clientSecret {
    | Some(clientSecret) =>
      let paymentIntentID = clientSecret->Utils.getPaymentId
      let headers = if !isSubscriptionFlow {
      ...
      let body = if isSubscriptionFlow {
      ...
      let path = if isSubscriptionFlow {
