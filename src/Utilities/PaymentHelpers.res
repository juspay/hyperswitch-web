  ...
  ->then(json => {
    ...
    if status === "succeeded" || status === "failed" {
      let outcome: SdkLogger.paymentOutcomeData = {
        status,
        manualRetryAllowed: dict->getBool("manual_retry_allowed", false),
      }
      SdkLogger.logLifecycle(
        ~event=status === "succeeded" ? PaymentSucceeded(outcome) : PaymentFailed(outcome),
        ~failure=?status === "failed" ? Some(json) : None,
...
let rec intentCall = (
  ...
  LoggerContext.setSessionData(
    ~paymentId=Utils.getPaymentIdOrExtractFromSdkAuth(~clientSecret, ~sdkAuthorization),
    (),
  )
        ...
        Promise.make(
          (resolve, _) => {
            if isConfirm {
              ...
              SdkLogger.logLifecycle(
                ~event=PaymentFailed({status: "failed", manualRetryAllowed: false}),
                ~failure=data,
        ...
        Promise.make(
          (resolve, _) => {
            ...
            let loggedPaymentMethod = LoggerTaxonomy.fromBackendPair(
              ...
              ~methodType=intent.payment_method_type,
            )
            ...
            if intent.status == "requires_customer_action" {
              if intent.nextAction.type_ == "redirect_to_url" {
                ...
                SdkLogger.logLifecycle(~event=QrCodeShown, ~paymentMethod=?loggedPaymentMethod)
                if !isPaymentSession {
                  messageParentWindow([
                    ("fullscreen", true->JSON.Encode.bool),
                ...
                SdkLogger.logLifecycle(
                  ~event=VoucherShown({downloads: 0}),
                  ~paymentMethod=?loggedPaymentMethod,
                )
                messageParentWindow([
                  ("fullscreen", true->JSON.Encode.bool),
                ...
                if uri->String.includes("force_sync=true") {
                  SdkLogger.logLifecycle(
                    ~event=RedirectUnsupported({nextAction: intent.nextAction.type_}),
                    ~paymentMethod=?loggedPaymentMethod,
                  )
                  handleOpenUrl(url.href)
                } else {
              ...
              let outcome: SdkLogger.paymentOutcomeData = {
                ...
                manualRetryAllowed: intent.manualRetryAllowed,
              }
  ...
  (
    ...
    | Some(clientSecret) =>
      ...
      let callIntent = body => {
        ...
        | Card =>
          SdkLogger.logLifecycle(
            ~event=PaymentRequested({bodySize: contentLength}),
...
let fetchEnabledAuthnMethodsToken = async (
  ...
  let response = await ClickToPayLogger.observeApi(
...
let fetchEligibilityCheck = async (
  ...
  let response = await ClickToPayLogger.observeApi(~event=EligibilityCheck, ~url=uri, ~call=() =>
    Fetch.fetch(
      uri,
      {
        method: #POST,
...
let fetchAuthenticationSync = async (
  ...
  let response = await ClickToPayLogger.observeApi(~event=AuthenticationSync, ~url=uri, ~call=() =>
    Fetch.fetch(
      uri,
      {
        method: #POST,
