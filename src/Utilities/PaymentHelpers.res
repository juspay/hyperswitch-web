...
let invokeKountSDK = (~fromScript) => {
  let {config} = Recoil.useRecoilValueFromAtom(RecoilAtoms.configAtom)
  let {publishableKey} = Recoil.useRecoilValueFromAtom(RecoilAtoms.keys)
  let kountEnabledKeys = GlobalVars.isProd
    ? ["pk_prd_9fd3349624bd46dcb0800d9fe79d2e8c", "pk_prd_8120ac99fa40419fb86537904097c93d"]
    : [
        "pk_snd_f54ef7b414f34ec58d01631462bf0275",
        "pk_snd_3b33cd9404234113804aa1accaabe22f",
        "pk_snd_457b57ec9c31496499aa26cb31f7732a",
      ]
  () => {
    try {
      let kountConfig: KountSDK.kountConfig = {
        clientID: "700000",
        environment: GlobalVars.isProd ? "PROD" : "TEST",
        isSinglePageApp: false,
      }
      let paymentId = (config.clientSecret->Js.String2.split("_secret_"))[0]
      if kountEnabledKeys->Js.Array2.includes(publishableKey) {
        if fromScript {
          Utils.handlePostMessage([
            ("invokeKountSDK", true->Js.Json.boolean),
            ("kountSid", paymentId->Js.Json.string),
          ])
        } else {
          let sdk = KountSDK.kountSDK(. kountConfig, paymentId)
          Js.log("Anti-fraud SDK activated!")
          let rec ddcStatus = () => {
            Js.Global.setTimeout(() => {
              Js.log(
                `DDC completed = ${sdk.\"IsCompleted"(.)
    ...
    | err => Js.log2(err, "KountSDK Error")
    }
  }
}
