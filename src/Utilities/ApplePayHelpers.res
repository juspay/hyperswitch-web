...
let startApplePaySession = (
  ...
  ssn.oncancel = _ => {
    applePaySessionRef := Nullable.null
    SdkLogger.logUser(~event=WalletSheetCancelled, ~paymentMethod=Wallet(ApplePay))
    handleFailureResponse(
      ...
      try {
        ...
        if (
          ...
          SdkLogger.logLifecycle(
            ~event=WalletInteraction({stage: "confirm_request_received"}),
            ~paymentMethod=Wallet(ApplePay),
...
let handleApplePayBraintreeClick = (
  ...
  let onSuccess = token => {
    if token == "" {
      ...
      SdkLogger.logLifecycle(
        ~event=WalletInteractionFailed({cause: "nonce_empty"}),
        ~paymentMethod=Wallet(ApplePay),
      )
    } else {
      SdkLogger.logLifecycle(
        ~event=WalletInteraction({stage: "payment_success"}),
        ~paymentMethod=Wallet(ApplePay),
      )
    ...
    braintreeClientCreate(
      ...
      (err, clientInstance) => {
        ...
        | None =>
          try {
            SdkLogger.logLifecycle(
              ~event=WalletInteraction({stage: "instance_created"}),
              ~paymentMethod=Wallet(ApplePay),
            )
            braintreeApplePayPaymentCreate(
              ...
              (err, applePayInstance) => {
                ...
                | None =>
                  SdkLogger.logLifecycle(
                    ~event=WalletInteraction({stage: "session_started"}),
                    ~paymentMethod=Wallet(ApplePay),
                  )
