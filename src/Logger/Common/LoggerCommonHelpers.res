...
let userGatedOperationTimeoutMs = 600000
...
let rec normalizeDetailJson = json =>
  ...
  | _ =>
    ...
    | String(value) =>
      value->truncateDiagnosticText(~maxLength=detailTextMaxLength)->JSON.Encode.string
    | Object(object) =>
      object
      ->Dict.toArray
      ->Array.filterMap(((key, value)) =>
        ...
        | Undefined => None
        | _ => Some((key->snakeCase, normalizeDetailValue(key->snakeCase, value)))
        }
      )
      ->Dict.fromArray
      ->JSON.Encode.object
...
and normalizeDetailValue = (key, value) =>
  switch (key, value->JSON.Decode.string) {
  | ("url" | "href", Some(text)) =>
    text
    ->sanitizedUrl
    ->truncateDiagnosticText(~maxLength=detailTextMaxLength)
    ->JSON.Encode.string
  | _ => value->normalizeDetailJson
  }
...
let normalizeDetails = (entries: details): details =>
  entries->Array.map(((key, value)) => {
    let normalizedKey = key->snakeCase
    (normalizedKey, normalizeDetailValue(normalizedKey, value))
  })

let positionalDetailKeys = ["target", "phase", "scope"]

let positionalDetails = (eventObject): details =>
  positionalDetailKeys
  ->Array.mapWithIndex((key, index) =>
    eventObject
    ->Dict.get(`_${index->Int.toString}`)
    ->Option.flatMap(payload =>
      switch JSON.Classify.classify(payload) {
      | String(value) => Some((key, value->screamingSnakeCase->JSON.Encode.string))
      | _ => None
      }
    )
  )
...
let variantEventMetadata = (event, ~includePositionalDetails=true): (string, details) => {
  ...
  | Some(eventObject) => {
      ...
      | Some(payload) =>
        switch payload->recordDetails {
        | [] => includePositionalDetails ? eventObject->positionalDetails : []
...
let rec axesSegmentsJson = (json, segments) =>
  switch JSON.Classify.classify(json) {
  | String(constructorName) => segments->Array.concat([constructorName->snakeCase])
  | Object(object) =>
    switch object->Dict.get("TAG")->Option.flatMap(JSON.Decode.string) {
    | Some(tag) =>
      axesSegmentsJson(
        object->Dict.get("_0")->Option.getOr(JSON.Encode.null),
        segments->Array.concat([tag->snakeCase]),
      )
    | None => segments
    }
  | _ => segments
...
let axesSegments = (value): array<string> => value->Identity.anyTypeToJson->axesSegmentsJson([])

let summarizeThrownValue = caughtError => {
  let caughtJson = caughtError->Identity.anyTypeToJson
  switch JSON.Classify.classify(caughtJson) {
  | String(value) => {
      name: "THROWN_VALUE",
      message: Some(value->truncateDiagnosticText(~maxLength=256)),
      details: [],
    }
  | Object(caughtObject) => {
      let findString = keys =>
        keys->Array.findMap(key => caughtObject->Dict.get(key)->Option.flatMap(JSON.Decode.string))
      {
        name: findString(["name", "code", "type", "reason", "statusCode"])
        ->Option.getOr("UNKNOWN_ERROR")
        ->truncateDiagnosticText(~maxLength=64),
        message: findString(["message", "description", "statusMessage"])->Option.map(message =>
...
let summarizeException = caughtError =>
  ...
  | _ => caughtError->summarizeThrownValue
  }
