...
// Logging convention: use PascalCase normal variants only. Context decides the wire casing:
// event/function/resource names become snake_case, while enum/detail values become SCREAMING_SNAKE_CASE.
...
let snakeCase = value =>
  value
  ->String.replaceRegExp(%re("/([A-Z]+)([A-Z][a-z])/g"), "$1_$2")
  ->String.replaceRegExp(%re("/([a-z0-9])([A-Z])/g"), "$1_$2")
  ->String.toLowerCase

let screamingSnakeCase = value =>
  value
  ->snakeCase
  ->String.replaceRegExp(%re("/[^a-zA-Z0-9]+/g"), "_")
...
// Convert record keys to snake_case and their string values to SCREAMING_SNAKE_CASE.
...
// Standard ReScript variants compile to either their constructor name or an object containing
// `TAG` and `_0`. Derive both event name and typed record details with no decorator requirement.
...
// Provider-owned events use one outer provider constructor and one inner event constructor.
// Extract both at the common boundary so domain loggers never format provider or target names.
...
let sanitizedUrl = url => url->String.replaceRegExp(%re("/[?#].*$/"), "")
