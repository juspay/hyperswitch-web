...
let runtimeSourceName = runtimeSource =>
  switch runtimeSource {
  | HyperLoader => "HYPER_LOADER"
  | Elements(paymentMode) =>
    `ELEMENTS_${paymentMode->CardThemeType.getPaymentModeToStrMapper->screamingSnakeCase}`
  | Headless => "HEADLESS"
  }

let configure = (~runtimeSource, ~component=?) => {
  runtime.source = runtimeSource->runtimeSourceName
  switch component {
  | Some(component) => runtime.component = component
  | None => ()
  }
}
...
let maxRetainedRows = 40

let maxDetailsSize = 8192

let consecutiveFlushFailures = ref(0)
...
let rec flush = () => {
  ...
  if runtime.rows->Array.length > 0 {
    ...
    if Window.Navigator.sendBeacon(GlobalVars.logEndpoint, body) {
      consecutiveFlushFailures := 0
      clearRows(runtime.rows)
    } else {
      consecutiveFlushFailures := consecutiveFlushFailures.contents + 1
      if consecutiveFlushFailures.contents >= 2 {
        runtime.rows->Array.splice(~start=0, ~remove=1, ~insert=[])
      }
      let overflow = runtime.rows->Array.length - maxRetainedRows
      if overflow > 0 {
        runtime.rows->Array.splice(~start=0, ~remove=overflow, ~insert=[])
      }
      scheduleFlush()
    }
  }
...
let log = (
  ...
  let exnDetails = switch exn {
  | Some(error) => {
      let summary = error->summarizeException
      let messageEntry = switch summary.message {
      | Some(message) => [("message", message->JSON.Encode.string)]
      | None => []
      }
      let exnKey =
        details->Array.some(((key, _)) => key->snakeCase === "error") ? "exception" : "error"
      [
        (
          exnKey,
          [("name", summary.name->JSON.Encode.string)]
          ->Array.concat(messageEntry)
  ...
  let normalizedDetails = {
    let candidate = details->Array.concat(exnDetails)->normalizeDetails
    let size = candidate->Dict.fromArray->JSON.Encode.object->JSON.stringify->String.length
    size > maxDetailsSize
      ? [("truncated", true->JSON.Encode.bool), ("original_size", size->JSON.Encode.int)]
      : candidate
  }
  ...
  if (
    ...
    let eventId = eventIdPrefix ++ LoggerCommonHelpers.generateRandomString(12)
    ...
    let value =
      [
        ("schema_version", 3->JSON.Encode.int),
    ...
    let row =
      [
        ...
        (
          "payment_method",
          paymentMethod
          ->Option.map(LoggerTaxonomy.paymentMethodName)
          ->Option.getOr("")
          ->JSON.Encode.string,
        ),
        (
          "payment_method_type",
          paymentMethod
          ->Option.flatMap(LoggerTaxonomy.paymentMethodTypeName)
          ->Option.getOr("")
          ->JSON.Encode.string,
        ),
      ]
...
let logEvent = (
  ...
  let (derivedEventName, eventDetails) =
    event->variantEventMetadata(~includePositionalDetails=eventNameOverride->Option.isNone)
  let eventName = eventNameOverride->Option.getOr(derivedEventName)
  safeRun(() =>
    log(
      ~category,
      ~severity,
...
let emitAsyncOutcome = (
  ...
  let outcomeName = outcome->LoggerCommonHelpers.outcomeName
  let subject = switch (category, provider) {
  | (Function | Merchant, Some(provider)) => `${provider->snakeCase}_${normalizedTarget}`
  | _ => normalizedTarget
  }
  log(
    ~category,
    ~severity=outcome->LoggerCommonHelpers.outcomeSeverity,
    ~eventName=`${action}.${subject}.${outcomeName}`,
    ~provider?,
    ~providerDimension,
    ~paymentMethod?,
    ~message?,
...
let logNamespacedEvent = (
  ...
  let (target, namespace, eventDetails) = event->providerEventMetadata
  let normalizedTarget = target->snakeCase
  let normalizedNamespace = namespace->snakeCase->String.replaceRegExp(/_prop$|_props$/, "")
  let subject = `${normalizedNamespace}_${normalizedTarget}`
  safeRun(() =>
    log(
      ~category,
      ~severity,
      ~eventName=`${action}.${subject}.set`,
      ~message?,
      ~details=eventDetails->Array.concat(details),
      ~taxonomyDetails=[
        ("action", action->JSON.Encode.string),
        ("subject", subject->JSON.Encode.string),
        ("outcome", "set"->JSON.Encode.string),
      ],
      ~eventIdPrefix,
    )
  )
