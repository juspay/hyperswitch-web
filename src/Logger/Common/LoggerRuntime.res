...
let log = (
  ...
  let normalizedDetails = details->normalizeDetails
...
let asyncOutcomeName = (category, outcome) =>
  ...
  | (Function | Merchant, Failed(_) | TimedOut(_)) => "failed"
...
let emitAsyncOutcome = (
  ...
  | (Function | Merchant, Some(provider)) => `${provider->snakeCase}.${normalizedTarget}`
  | _ => normalizedTarget
  }
  log(
    ~category,
...
let logNamespacedEvent = (
  ...
  safeRun(() =>
    log(
      ~category,
      ~severity,
      ~eventName=`${action}.${normalizedNamespace}.${normalizedTarget}`,
      ~message?,
      ~details=eventDetails->Array.concat(details),
      ~taxonomyDetails=[
        ("action", action->JSON.Encode.string),
        ("namespace", normalizedNamespace->JSON.Encode.string),
        ("target", normalizedTarget->JSON.Encode.string),
      ],
      ~eventIdPrefix,
    )
