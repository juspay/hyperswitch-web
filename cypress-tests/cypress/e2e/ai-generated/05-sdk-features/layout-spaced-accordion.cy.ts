    ...
    it("should have margin-bottom between accordion items (spaced mode)", () => {
      getIframeBody()
        ...
        .then(($el) => {
          ...
          // spacedAccordionItems: true should add spacingAccordionItem (default 10px)
    ...
    it("should have individual border-radius on each item (not shared edges)", () => {
      getIframeBody()
        ...
        .then(($el) => {
          ...
          // Should have some border-radius (default theme is "4px")
    ...
    it("should switch selection when clicking a different item", () => {
      getIframeBody().find(".AccordionItem").then(($items) => {
        if ($items.length > 1) {
          ...
          // AccordionItem--selected is on an inner child div, not the outer .AccordionItem
          getIframeBody()
            .find(".AccordionItem")
            .eq(1)
            .find(".AccordionItem--selected")
