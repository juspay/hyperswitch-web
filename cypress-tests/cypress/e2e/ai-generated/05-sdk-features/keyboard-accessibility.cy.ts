...
describe("Keyboard Navigation and Accessibility", () => {
  ...
  beforeEach(() => {
    ...
    // Reset shared mutable state that may have been modified by prior test files
    changeObjectKeyValue(
      createPaymentBody,
      "profile_id",
  ...
  describe("Tab Navigation", () => {
    it("should allow tabbing from card number to expiry field", () => {
      ...
      // After completing card number, focus should auto-advance to expiry
      // (The SDK auto-advances focus when card number is complete)
      getIframeBody()
    ...
    it("should allow tabbing from expiry to CVC field", () => {
      ...
      // After completing expiry, focus should auto-advance to CVC
      getIframeBody()
    ...
    it("should allow completing full payment using only keyboard", () => {
      ...
      // Type card number (auto-advances to expiry)
      getIframeBody()
        .find(`[data-testid=${testIds.cardNoInputTestId}]`)
        .safeType(cardNo);

      // Type expiry (auto-advances to CVC)
      getIframeBody()
        .find(`[data-testid=${testIds.expiryInputTestId}]`)
  ...
  describe("Error Announcement", () => {
    it("should display visible error messages on invalid submission", () => {
      // Type a partial/invalid card number to trigger client-side validation error
      getIframeBody()
        .find(`[data-testid=${testIds.cardNoInputTestId}]`)
        .safeType("4242 4242");

      // Type incomplete expiry
      getIframeBody()
        .find(`[data-testid=${testIds.expiryInputTestId}]`)
        .safeType("12");
    ...
    it("should clear error message when user starts correcting input", () => {
      ...
      // Start typing valid card number
      getIframeBody()
        .find(`[data-testid=${testIds.cardNoInputTestId}]`)
        .safeType(stripeCards.successCard.cardNo);

      // Verify the card number field now has a value
      getIframeBody()
        .find(`[data-testid=${testIds.cardNoInputTestId}]`)
