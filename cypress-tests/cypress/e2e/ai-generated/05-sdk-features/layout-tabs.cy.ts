...
describe("Layout - Tabs (Default)", () => {
  ...
  beforeEach(() => {
    ...
    // Restore billing in case a previous test removed it
    ...
    cy.createPaymentIntent(secretKey, createPaymentBody).then(() => {
      cy.getGlobalState("clientSecret").then((clientSecret) => {
        // Default layout is tabs — no layout param needed
  ...
  describe("Overflow Dropdown", () => {
    it("should render overflow dropdown if there are more payment methods than visible tabs", () => {
      // The dropdown may or may not be present depending on available payment methods
      // and viewport width. We verify the structure when present.
