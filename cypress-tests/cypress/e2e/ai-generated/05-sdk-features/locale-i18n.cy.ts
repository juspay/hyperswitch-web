...
describe("Locale / i18n Tests", () => {
  ...
  beforeEach(() => {
    ...
    // Restore shared mutable state to defaults before each test
    changeObjectKeyValue(
      createPaymentBody,
      "profile_id",
  ...
  describe("Expiry Placeholder Localisation", () => {
    // The demo app uses `labels: "floating"` appearance mode.
    // In Floating mode, PaymentInputField sets placeholder="" (empty string)
    // and shows the field name as a floating label instead.
    // We verify the expiry input exists and the locale-specific label is shown
    // as a floating label (the label text is tested in "Card Field Label Translations").
    locales.forEach(({ code, name, validThruText }) => {
      it(`should show expiry floating label "${validThruText}" in ${name} (${code})`, () => {
        setupWithLocale(code, secretKey, publishableKey);

        // Verify the expiry input is present
        getIframeBody()
          .find(`[data-testid=${testIds.expiryInputTestId}]`)
          .should("be.visible");

        // In Floating mode, the field label appears as a floating label div
        // adjacent to the input. Verify the translated label text is visible.
        getIframeBody().contains(validThruText).should("be.visible");
      });
    });
  });
  ...
  describe("Error Message Translations", () => {
    locales.forEach(({ code, name, cardNumberEmptyText }) => {
      it(`should display translated empty-card error in ${name} (${code})`, () => {
        ...
        // Fill only expiry and CVC, leave card number empty
        getIframeBody()
          .find(`[data-testid=${testIds.expiryInputTestId}]`)
          .safeType("1230");
  ...
  describe("RTL Layout Support", () => {
    ...
    rtlLocales.forEach(({ code, name }) => {
      it(`should apply RTL direction for ${name} (${code})`, () => {
        ...
        // RenderPaymentMethods.res sets dir=localeString.localeDirection on a
        // child <div> inside the iframe body (not on <body> itself).
        // We must look for [dir] on child elements.
        getIframeBody()
    ...
    ltrLocales.forEach(({ code, name }) => {
      it(`should apply LTR direction for ${name} (${code})`, () => {
        ...
        // RenderPaymentMethods.res sets dir=localeString.localeDirection on a
        // child <div> inside the iframe body.
        getIframeBody()
          .find("[dir]", { timeout: 10000 })
          .first()
  ...
  describe("Billing Field Label Translations", () => {
    ...
    billingLocales.forEach(({ code, name, billingDetailsText }) => {
      it(`should display translated billing details header in ${name} (${code})`, () => {
        // Use Cybersource without billing to trigger dynamic fields
        changeObjectKeyValue(
          createPaymentBody,
          "profile_id",
  ...
  describe("Locale Fallback Behaviour", () => {
    ...
    it("should fall back to base language when regional variant is unsupported", () => {
      // "fr-CA" (French-Canada) is not in the locale files; should fall back to "fr"
    ...
    it("should fall back to English for empty locale string", () => {
      // Empty locale should behave like auto or default
      changeObjectKeyValue(
        createPaymentBody,
        "customer_id",
        "locale_empty_test",
      ...
      cy.createPaymentIntent(secretKey, createPaymentBody).then(() => {
        cy.getGlobalState("clientSecret").then((clientSecret) => {
          // Visit without locale param (getClientURL with undefined locale)
      ...
      // Should render in browser language (likely English in CI)
      getIframeBody()
  ...
  describe("SDK Loads for All Supported Locales", () => {
    ...
    allLocales.forEach((code) => {
      it(`should load SDK without errors for locale "${code}"`, () => {
        ...
        // Card number input should be present (SDK rendered successfully)
        getIframeBody()
          .find(`[data-testid=${testIds.cardNoInputTestId}]`)
          .should("be.visible");
